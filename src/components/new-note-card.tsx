...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/50 0" />
        <Dialog.Content className="fixed overflow-hidden left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col outline-none">
          <Dialog.Close className="hover:text-slate-100 absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400">
            <X className="size-5" />
          </Dialog.Close>
          ...
          <form className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              <button
                ...
                className="font-medium w-full flex items-center flex-center justify-center gap-2 bg-slate-900 py-4 text-center text-sm text-slate-300 outline-none hover:text-slate-100"
              >
